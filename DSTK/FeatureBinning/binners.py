...
class ConditionalInferenceBinner(object):
    ...
    def __init__(self, name, **kwargs):
        ...
        self._splits = list()
        self._leafs = list()
        self.children_left = list()
        self.children_right = list()
        self.nodes = list()
        self.cond_proba_buckets = list()
    ...
    def fit(self, values, targets):
        ...
        self._recurse(values, targets, 0)
        self._create_bins(values, targets)
        self.cond_proba_buckets.append((np.NaN, [0.5, 0.5]))
        self.is_fit = True
    ...
    def _create_bins(self, values, targets):
        ...
        for bin_idx, bin in enumerate(np.unique(idx)):
            ...
            if bin_idx == 0:
                ...
                self.cond_proba_buckets.append(((self._splits[bin_idx-1], self._splits[bin_idx]), list(counts / counts.sum())))
    ...
    def _recurse(self, values, targets, node_id):
        ...
        if self._terminate_recursion(new_vals, new_targets):
            ...
            if split:
                ...
                self.children_left.append(node_id + 1)
                self._recurse(new_vals[left_idx], new_targets[left_idx], node_id=node_id+1)

                self.children_right.append(node_id + 2)
                self._recurse(new_vals[right_idx], new_targets[right_idx], node_id=node_id+2)

                self.nodes.append(Node(node_id, split, node_id + 1, node_id + 2, False, counts, statistic))
            else:
                self._create_leaf(node_id, counts, statistic)
    ...
    def _create_leaf(self, node_id, counts, statistic):
        ...
        self._leafs.append(node_id)
        self.nodes.append(Node(node_id, np.NaN, -1, -1, True, counts, statistic))
